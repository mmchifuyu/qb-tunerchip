...
function setVehData(veh, data, pbool)
    local multp =  0.12
    local dTrain = 0.0 -- Do not change this
    local boost = 0.0  -- Do not change this
    local acceleration = 0.10 -- 
    local gearchange = 0.12  --

    if tonumber(data.boost) >= 0.2 then boost = 0.12 elseif tonumber(data.boost) >= 0.3 then boost = 0.10 end -- 
    ...
    SetVehicleHandlingFloat(veh, "CHandlingData", "fInitialDriveForce", data.boost * boost) --
    SetVehicleHandlingFloat(veh, "CHandlingData", "fDriveInertia", data.acceleration * acceleration) --
    SetVehicleEnginePowerMultiplier(veh, data.gearchange * gearchange) --
    ...
    if pbool then
        TriggerServerEvent('limbo-tunerchip:saveSettings', GetVehicleNumberPlateText(veh), data.boost, data.acceleration, data.gearchange, data.breaking)
    end
end

RegisterNetEvent('limbo-tunerchip:getSettings', function(veh, data)
    setVehData(NetworkGetEntityFromNetworkId(veh), data, false)
end)
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    QBCore.Functions.Notify('vehicle Tune Has Been Reset!', 'error')
end)
...
RegisterNetEvent('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        QBCore.Functions.Progressbar("connect_laptop", "Connecting tunerchip to panel", 100, false, true, {
...
RegisterNetEvent('qb-tunercheap:setbackGround', function(url)
    local cid = QBCore.Functions.GetPlayerData().citizenid
    SetResourceKvp(cid, url)
    SendNUIMessage({
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    local kvpValue = GetResourceKvpString(QBCore.Functions.GetPlayerData().citizenid)
    if not kvpValue then return end
    TriggerEvent('qb-tunercheap:setbackGround', kvpValue)
end)